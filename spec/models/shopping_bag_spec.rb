...
RSpec.describe ShoppingBag, type: :model, site_scoped: true do
  ...
  context '#place_order' do
    ...
    context 'with a product' do
      ...
      before { subject.apply(product_id: product.id, quantity: 1) }
    ...
    context 'with a product with metadata' do
      ...
      before do
        subject.apply(product_id: product.id, quantity: 1,
                      metadata: { 'a' => 'z' })
      end
    ...
    context 'with a variant' do
      ...
      before { subject.apply(variant_id: variant.id, quantity: 1) }
    ...
    context 'with a variant with metadata' do
      ...
      before do
        subject.apply(variant_id: variant.id, quantity: 1,
                      metadata: { 'a' => 'z' })
      end
